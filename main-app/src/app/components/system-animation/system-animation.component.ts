...
export class SystemAnimationComponent extends RxUnsubscribe implements OnInit, OnChanges {
    ...
    public onTimeChange(x: number): void {
        ...
        const servingCustomers = this.model.customer_data.filter((c) => c.serve !== null && c.serve <= x && c.leave > x);
        ...
        const queuedCustomers = this.model.customer_data.filter((c) => c.arrive <= x && c.serve > x).filter((_, i) => i < 4);
